...
object flinkContinuousProcessingRollover {
        ...
        val _rolloverSink02: FileSink[String] = FileSink
          ...
          .withRollingPolicy(
              DefaultRollingPolicy.builder()
                ...
                .withMaxPartSize(MemorySize.ofMebiBytes(10)) //Approx 10 MB
                .build())
          .build()

        //Avro
        val _schema = "{'doc': 'NYSE prices','name': 'NYSE','namespace': 'stocks','type': 'record','fields': [{'name': 'xchange', 'type': 'string'},{'name': 'symbol', 'type': 'string'},{'name': 'trdate', 'type': {'type': 'string', 'logicalType': 'time-millis'}},{'name': 'open', 'type': 'float'},{'name': 'close', 'type': 'float'},{'name': 'low', 'type': 'float'},{'name': 'high', 'type': 'float'},{'name': 'volume', 'type': 'int'}{'name': 'adj_close', 'type': 'float'}]}"
